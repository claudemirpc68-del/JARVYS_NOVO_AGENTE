...
try:
    ...
    # Suportar tanto tipo 'web' quanto 'installed'
    ...
    if not CLIENT_ID or not CLIENT_SECRET or "seu-client-id" in CLIENT_ID:
        raise ValueError("credentials.json contem placeholders ou esta vazio. Configure o arquivo primeiro.")
except Exception as e:
    print(f"\n[ERRO] Ao ler credentials.json: {e}")
    print("Certifique-se de que o arquivo 'credentials.json' existe e contem suas credenciais reais do Google.")
    sys.exit(1)
...
auth_url = (
    ...
    f"&redirect_uri=http%3A%2F%2Flocalhost%3A8000"
